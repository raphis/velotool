...
$item = ['name' => '', 'manufacturer' => '', 'price_chf' => '', 'note' => '', 'stock_qty' => 0, 'stock_note' => ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        if ($id) {
            $pdo->prepare('UPDATE parts_catalog SET name=?, manufacturer=?, price_chf=?, note=?, stock_qty=?, stock_note=? WHERE id=?')
                ->execute([...array_values($data), $id]);
        } else {
            $pdo->prepare('INSERT INTO parts_catalog (name, manufacturer, price_chf, note, stock_qty, stock_note) VALUES (?, ?, ?, ?, ?, ?)')
